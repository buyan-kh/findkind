...
async def search_pets(
    description, image_data, custom_pets=None, models=None, location=None
):
    ...
    found_text_embedding = get_cached_embedding(
        f"text_{description}", generate_text_embedding
    )

    def generate_db_text_embeddings():
        return local_text_model.encode(
            [entry["description"] for entry in filtered_pets], convert_to_tensor=True
        )

    db_key = "_".join(
        [
            str(pet.get("id", hash(json.dumps(pet, default=str))))
            for pet in filtered_pets
        ]
    )
    text_embeddings = get_cached_embedding(
        f"db_text_{db_key}", generate_db_text_embeddings
    )
    ...
    if image_data:
        ...
        if found_image:
            ...
            found_image_embedding = get_cached_embedding(
                f"img_{image_data}", generate_image_embedding
            )
            ...
            for i, pet in enumerate(filtered_pets):
                grid_out = await db.fs_bucket.open_download_stream(
                    ObjectId(pet["image_id"])
                )
                pet_image_data = await grid_out.read()
                pet_image = (
                    Image.open(BytesIO(pet_image_data))
                    .convert("RGB")
                    .resize((128, 128))
                )
                if pet_image:

                    def generate_pet_embedding(
                        pet_img=pet_image, pet_id=pet.get("id", "")
                    ):
                        return local_image_model.encode(pet_img, convert_to_tensor=True)

                    embedding = get_cached_embedding(
                        f"img_{pet.get('id', hash(str(pet.get('image_url', ''))))}",
                        generate_pet_embedding,
                    )
            ...
            if image_embeddings:
                ...
                image_scores = util.cos_sim(
                    found_image_embedding, image_embeddings_tensor
                )[0]
    ...
    results = sorted(
        result, key=lambda x: x.get("combined_score", x["text_score"]), reverse=True
    )
    ...
    if not results:
        ...
        for match in results[:3]:
            ...
            if "location" in match:
                print(
                    f"Location: Lat {match['location'].get('lat')}, Lon {match['location'].get('lon')}"
                )
            print()
